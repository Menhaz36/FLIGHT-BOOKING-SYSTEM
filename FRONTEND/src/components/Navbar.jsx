...
const Navbar = () => {
    ...
    return (
        <nav className={`fixed top-0 left-0 bg-black w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
            ...
            `}</style>
            <div className="relative inline-block p-0.5 rounded-full overflow-hidden hover:scale-105 transition duration-300 active:scale-100 before:content-[''] before:absolute before:inset-0 before:bg-[conic-gradient(from_0deg,_#FFFFFF,_#FFFFFF30,_#FFFFFF)] button-wrapper">
                <button className="relative z-10 bg-black text-white rounded-full px-8 py-3 font-medium text-sm">Login</button>
            </div>
